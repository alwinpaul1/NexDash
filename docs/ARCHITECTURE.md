# NexDash — Architecture & Detailed Reference

> **About this document.** This is the full architecture / design reference for NexDash.
> It was previously the project README; it has been moved here so the README can stay
> short and run-focused. Everything below — physics, features, evaluation, agent design,
> long-term strategy — is the detailed version. For a quick start and the four case-study
> sections in brief, see the top-level [`README.md`](../README.md).
...
The dispatcher talks to MiniMax through an OpenAI-compatible adapter, so the tool layer and tests stay provider-agnostic. The physics, dataset, model and evaluation all work **entirely offline** — only the LLM agent/CLI need a key; without one, `/api/chat` degrades to a friendly message rather than failing.
...
NexDash ships an MCP server (`nexdash.mcp_server`) exposing `predict_energy` and `check_reachability` as tools. Add it to any MCP host like so:
...
├── docs/
│   ├── ARCHITECTURE.md        # This document (detailed reference, formerly the README)
...
**Ground truth: physics.** `segment_energy_kwh(...)` adds up the four classic loads, then divides the total by drivetrain efficiency:

- **Rolling resistance.** The energy to overcome the tyres rolling on the road. It scales with total mass (kerb plus payload) and distance. Its coefficient starts from a base value and is bent by speed and temperature: a modest rise above 80 km/h and cold-tyre stiffening below 20 °C, both normalised to 1.0 at the 80 km/h / 20 °C reference.
- **Aerodynamic drag.** The energy to push air aside, the dominant load at motorway pace. It scales with the square of the effective air speed (road speed plus any headwind) and with distance. Air density itself depends on temperature, so cold air (about 1.341 kg/m³ at −10 °C) costs more drag than warm air (about 1.146 at 35 °C), with 1.225 at 15 °C as the pivot.
- **Gradient (potential energy).** The energy to lift the truck's mass over the climb. Downhill segments recover a base 60% of that energy through regenerative braking, which is why steep descents can net *negative* kWh. The recovery is tapered on descents by temperature (down to a floor of 45% at −15 °C, for cold battery charge-acceptance) and by grade (down to a floor of 70% at −10% grade, where steep descents exceed the regen power cap and bleed the rest to friction braking).
- **Auxiliary / HVAC.** A base load plus cabin heating or cooling that rises at both cold and hot extremes, scaled by how long the leg takes.
...
| --- | --- | --- |
| +8 % | **473.2** | uphill — lifting ~33 t gross up the climb dominates |
| +4 % | 262.8 | uphill |
| 0 % (flat) | 51.4 | baseline rolling + aero + aux |
| −4 % | **−40.3** | downhill — regen returns more than the segment spends (net negative) |
| −8 % | −98.6 | steeper descent recovers more |
...
**Features.** The model consumes the six raw inputs — `distance_km, payload_t, speed_kph, gradient_pct, temperature_c, wind_mps` — plus **six engineered terms** (12 features total) that encode the physics structure so a tree model finds the patterns faster: **|gradient|** (`abs_gradient`), **temperature deviation from 20 °C** (`temp_dev_from_20`; HVAC is a U-shape, not linear), a **payload × gradient** interaction (`payload_x_gradient`; mass amplifies grade), a **speed² proxy** (`speed_sq`; drag is quadratic), **payload × distance** (`payload_x_distance`; rolling-resistance energy), and **distance × signed gradient** (`distance_x_gradient`; the signed gravity-work term that scales climb/descent energy). A `net_climb_km` term (the leg's net vertical climb) was **removed**: it was ~99.9 % collinear with `distance_x_gradient` (the small trigonometric correction it added over that term is ≤0.18 % even at 6 %) and did *not* cure climb saturation — a tree cannot extrapolate a linear quantity past its training labels however cleanly that quantity is presented. Each feature is documented in `nexdash.features`.

**Estimator + physics-residual target (the real saturation fix).** A `HistGradientBoostingRegressor` is the production model, but it regresses the **physics residual** (the gap between the measured energy and the physics estimate) rather than raw kWh, and reconstructs the physics estimate plus that residual at inference (`nexdash.model`). This makes physics the structural backbone: it carries the dominant linear gradient/distance work *analytically* (so it extrapolates correctly past the ~1500 m net-climb ceiling and the ~350 km distance max), while the bounded, near-zero-mean residual captures the data-driven correction where samples are dense. A raw-kWh tree *saturated* (flat-lined, under-predicting versus physics) on out-of-envelope steep/long inputs — the strand-the-truck direction (a 100 km / +6 % leg read ~289 kWh vs physics ~805, −64 %; a flat 700 km leg flat-lined at ~231 vs physics ~799, −71 %). The residual reparametrisation removes that by construction (those now track physics to ~1 %). A small steep-short densification stratum (~12 % of rows, full ±6 % grade over ≤30 km legs) gives the residual tree real data in the steep-grade quadrant the net-climb cap otherwise starves. A `LinearRegression` baseline is still trained on raw kWh so every report shows *how much the non-linear model actually buys us*. `EnergyModel` wraps a scikit-learn pipeline and accepts raw feature dicts (it calls `features.transform` internally), so callers never duplicate engineering logic.

Full rationale, formulas and assumptions: **[docs/DESIGN.md](DESIGN.md)**.

---

## Evaluation

`run_pipeline.py` writes `reports/evaluation_report.md` with **real, regenerated numbers** plus figures. The values below are the **actual results** from the committed run (`n=6000`, `seed=42`, 4,800 train / 1,200 held-out test). The truck spec and synthetic operating envelope are now **calibrated to real Mercedes-Benz eActros 600 figures** (German long-haul ops) — see [`docs/REAL_WORLD_CALIBRATION.md`](REAL_WORLD_CALIBRATION.md) for every cited assumption.

**Held-out test set (gradient-boosting primary, n=1,200):** MAE **4.426 kWh**, RMSE 8.010 kWh, MAPE 6.73 %, R² 0.9913. (The physics-residual reparametrisation pulled MAE down from the prior 5.661 kWh and eliminated the out-of-envelope saturation; see the failure section.)

The headline metric is the **MAE in kWh (4.43)** — that is the number a dispatcher should reason about per segment. We also report a *fleet-intuitive* proxy: 4.43 kWh is **0.74 %** of the eActros 600's ~600 kWh usable battery (the energy it spends across its ~500 km real-world range). This proxy is flattering on long segments (whose absolute energies are large), so treat the per-kWh MAE — and the per-regime failure table below — as the real accuracy signal, not the percentage.

**Read these numbers honestly.** Two framings keep them grounded:

- **Per-SEGMENT, not per-trip.** Every figure above is the error on a *single leg*. A real route is many segments and the errors accumulate (roughly with the number of legs), so the **trip-level** error is several times larger than any single-segment figure and is **not** bounded by the 0.74 % proxy. Treat that percentage as a per-leg sanity scale, never a promise that a whole route lands inside a 10 % reserve.
...
The physics-residual gradient-boosting model now lands MAE 4.43 kWh — about a third of the linear baseline (12.21 kWh; the baseline still regresses raw kWh). The signed `distance_x_gradient` engineered feature (the gravity-work term) helps localise the residual; the dominant climb/descent scaling now comes from the physics backbone rather than being re-learned by the tree, which is what removes the old saturation. Three diagnostic figures are saved to `reports/figures/`: **predicted-vs-actual**, **residual-vs-temperature**, and **error-by-payload**.
...
- **Steep climbs (gradient > +4 %).** This is the dangerous slice. Held-out MAE versus the noisy labels is **6.60 kWh** (n=49), but versus *clean* physics it is only **~2.6 kWh** with near-zero bias — so ~86 % of the headline is the irreducible ~6 % multiplicative label noise on this slice's ~115 kWh-mean labels, not model error. The decisive change is that the old climb **saturation** is gone: the raw-kWh model under-predicted these out-of-envelope legs by up to −64 % versus physics; the physics-residual model now tracks physics into the steep tail (a 100 km / +6 % leg: ~811 kWh model vs ~805 physics). The `max(model, physics)` guard still backstops the slice.
- **Heavy payload (> 15 t).** MAE **5.21 kWh** (n=378) — the worst-error payload bin. Payload scales rolling resistance and gradient potential energy linearly, so heavy loads both consume the most and leave the most room to be wrong in absolute terms.
- **Steep descents (gradient < −4 %).** Small absolute error (MAE 1.07 kWh, n=42) and a genuine regime: these net-regen segments carry **real negative labels** (no zero-clamp), so the regen signal is preserved. The residual model **credits regen correctly even on long descents** now — the old raw-kWh model floored the credit near zero on a 100 km / −6 % leg (a −179 kWh credit gap versus physics); the residual model lands within ~3 kWh. Its *percentage* error is loud (MAPE 8.43 %) because regen drives net energy near zero and the denominator collapses — which is precisely why we rank by MAE, not MAPE.
- **Long-and-steep pocket (auto-discovered).** `failure_miner` flags `gradient_pct>0.5 AND distance_km>~124` at MAE **18.4 kWh** (4.16× lift, n=30). This is **not** saturation: ~88 % of that MAE is the ~6 % label noise on the pocket's ~287 kWh-mean labels; the model's true error versus *clean* physics there is ~6.6 kWh (~2.3 %) with a mildly conservative bias. The pocket looks alarming only because its absolute energies — and hence the proportional noise floor — are large.
- **Temperature.** Cold is **not** the hardest regime to *predict*, even though the upgraded physics makes it the most *expensive*: the cold consumption channels (denser air + cold tyres + reduced regen) are smooth and learnable, so the error slices stay flat — cold MAE 4.75 (n=175), mild 4.44 (n=962), hot 3.32 (n=63). A winter run costs more energy (see below) but is not meaningfully harder to predict than a mild one; don't conflate the two.

**Direct physics-vs-ML check at the tail.** Running the trained model against its own physics teacher on representative segments makes the fix concrete. At the **scale the planner actually scores energy** — ~25 km chunks — the two agree closely across the full ±6 % grade range, climbs and descents alike: a 25 km / +6 % chunk reads 198 vs physics 201 kWh (−2 %), a 25 km / −6 % regen chunk −42 vs −42 kWh (both genuinely negative). More importantly, the **physics-residual target removed the out-of-envelope saturation that the prior raw-kWh model suffered** — and removed it *structurally*, not just at the ~25 km chunk scale. The old model flat-lined (under-predicting versus physics) on sustained steep climbs and on distances past the ~350 km training max: a 100 km / +6 % leg read ~289 kWh vs physics ~805 (−64 %); a flat 700 km leg flat-lined at ~231 vs physics ~799 (−71 %). Because physics now carries the linear gradient/distance work analytically and the tree only learns the bounded residual, those same legs now read ~811 and ~805 kWh — within ~1 % of physics.

What used to be the worst case — a sustained 50 km × +5 % leg (~2500 m net climb, steeper than any real ~25 km planner chunk and outside the training set) — previously under-predicted by ~−31 % (≈221 model vs ≈322 physics). With the residual reparametrisation it now reads ≈353 vs ≈345 physics (+2 %): the tree can no longer saturate *below* physics because physics is the structural backbone, not a quantity the piecewise-constant tree must extrapolate. The long steep **descent** credit the old model floored near zero (a −179 kWh gap on a 100 km / −6 % leg) is likewise recovered to within ~3 kWh. The planner never *builds* such a long single chunk anyway (it walks the route in ~25 km windows), and the `max(model, physics)` cross-check in `range.check_reachability` and the per-chunk planner walk **stays in place as defense-in-depth** — it now fires far less often (zero divergence triggers on the steep ≥3 % / ≥25 km grid that used to trip it), which is the observable proof the model improved without weakening the guard.
...
- **Air density** now depends on temperature (it was a constant 1.225). It runs about 1.341 kg/m³ at −10 °C versus 1.146 at 35 °C, with 1.225 at 15 °C as the pivot.
- **Rolling resistance** now rises with speed and with cold (it was a flat 0.0055, independent of both).
- **Regen** is now tapered on descents by temperature and grade (floors of 45% in the cold and 70% on steep grades; it was a flat 60%). The steep-grade floor is 70% rather than something lower so that recovered energy never drops as the grade steepens: a steeper descent must never return *less* charge.
...
- **No absolute altitude / pressure.** The air-density model assumes sea level, so alpine hauls slightly **over-state cold-air drag** (Germany is mostly 0–1000 m, so this is a known *one-sided* bias). Fixing it needs an elevation input and a dataset regeneration.
...
- **Grade-only regen proxy.** The regen power-cap taper keys off **grade**, but the true motor-cap knee depends on the actual gravitational power (mass, slope and speed together), not grade alone.
...
The candid framing stands: "no weaknesses" is impossible. We fixed the big four and the bullets above name what remains. The steep-up slice is small (n=36; treat as indicative), rare *combinations* of features are still under-represented, and accuracy must be re-earned on real telemetry — the synthetic numbers establish that the pipeline works, not that it is calibrated to a real eActros.
...
- **Tools (`nexdash.tools`).** Three tool schemas — `predict_energy`, `check_reachability`, and `plan_route` — with thin, JSON-serializable wrappers and a `dispatch(name, args)` router. The model is *forbidden in the system prompt from inventing numbers*; it must call a tool.
- **Agent (`nexdash.agent.DispatcherAgent`).** `ask(question)` sends the question plus `TOOL_SPECS` to MiniMax-M3, executes any tool calls via `dispatch`, feeds the result back, and loops (up to 8 turns) until the model returns plain-language text. MiniMax is a reasoning model, so its `<think>…</think>` is stripped from the user-facing reply (kept in history for continuity). The system prompt frames it as a fleet dispatcher that always states the **margin** and a **caveat**. The client is injectable, so tests run fully mocked with no network.
- **MCP (`nexdash.mcp_server`).** The `predict_energy` and `check_reachability` tools are registered on a `FastMCP` server named `nexdash`, delegating to `nexdash.tools` so there is no logic drift — any MCP-capable host gets NexDash's reasoning without importing Python.
- **CLI (`nexdash.cli`).** A friendly REPL (or `--once`) over the agent, with a clear message and non-zero exit code if `MINIMAX_API_KEY` is not set.
- **HTTP + chat UI (bonus).** `POST /api/chat` wires `DispatcherAgent.ask` over the same loop, returning the reply plus which tools were used, and degrades gracefully (no crash) when no key is set or the provider is rate-limited. The React console in `frontend/` exposes this as a floating **chat widget** (`ChatWidget` / `ChatPanel`) that **renders the agent's Markdown** (tables, lists, caveat) and can plan a whole trip from plain language — the brief's "chat UI is a plus."
...
The synthetic-physics approach is a launchpad, not the destination. As real eActros telemetry arrives, NexDash is designed to swap the data source without touching the model interface. The strategy — **continuous retraining, model versioning, and drift detection** — is detailed in **[docs/LONG_TERM.md](LONG_TERM.md)**, and the core of it is now **implemented and runnable**, not just described:
